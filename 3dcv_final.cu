...
/*
 * ============================================================================
 * THEIA POINT CLOUD PROCESSING - Command-Line Usage
 * ============================================================================
 * 
 * This program processes PLY point cloud files using gaze-based foveated
 * rendering with log-polar transformation.
 * 
 * USAGE:
 *   program.exe [input_file] [output_file] [gaze_pos_x] [gaze_pos_y] [gaze_pos_z] 
 *               [gaze_dir_x] [gaze_dir_y] [gaze_dir_z]
 * 
 * PARAMETERS:
 *   input_file     - Path to input PLY file (default: longdress_vox10_1052.ply)
 *   output_file    - Path to output PLY file (default: theia_clean_output.ply)
 *   gaze_pos_x/y/z - Gaze position coordinates (default: 0.0, 1.5, -1.0)
 *   gaze_dir_x/y/z - Gaze direction vector (default: 0.0, 0.0, -1.0)
 * 
 * EXAMPLES:
 *   1. Use default parameters:
 *      ./3dcv_final
 * 
 *   2. Specify only input file:
 *      ./3dcv_final mymodel.ply
 * 
 *   3. Specify input and output files:
 *      ./3dcv_final mymodel.ply result.ply
 * 
 *   4. Custom files with gaze position:
 *      ./3dcv_final mymodel.ply result.ply 0.5 2.0 -0.5
 * 
 *   5. Full parameter specification:
 *      ./3dcv_final mymodel.ply result.ply 0.0 1.5 -1.0 0.0 0.0 -1.0
 * 
 *   6. View help:
 *      ./3dcv_final --help
 * 
 * OUTPUT:
 *   - theia_clean_output.ply - Processed point cloud file
 *   - Console output with timing and statistics
 * 
 * NOTE:
 *   - Input PLY file must be in ASCII format with x,y,z,r,g,b properties
 *   - Gaze direction vector will be automatically normalized
...
__global__ void logPolarTransformKernel(Point* points, int numPoints, float radius_min, int r_bins, int theta_bins, int phi_bins, Point_rgb* logPolarBuffer, Gaze gaze, Vector3d rightVector, Vector3d upVector, float rate_adapt)
{
    ...
    //atomicAdd(&logPolarBuffer[tmp_position].r, 100);
    ...
    atomicMinn(&logPolarBuffer[tmp_position].depth_val, d); // Still for minimum depth
...
//std::vector<Point> get_fovea(BYTE * pp_pc, int dataLen, Gaze gaze, Point* d_points){
  ...
  //std::cout << "Depth min_max time used:" << t3-t1 << std::endl;
    ...
    //float base = (theta_bins + M_PI) / (theta_bins - M_PI);
    //, float rate_adapt
		int inner_point_idx = (int)(log(tan(0.15 * M_PI / 180.0) / radius_min ) / log(base) / rate_adapt) * theta_bins;
...
int main(int argc, char* argv[]) {
    printf("=== THEIA STANDARD PROCESSING (Test 3 Config) ===\n");

    // Parse command-line arguments with defaults
    const char* filename = "longdress_vox10_1052.ply";
    const char* output_filename = "theia_clean_output.ply";
    ...
    if (argc > 3) {
        gaze_pos_x = atof(argv[3]);
    }
    if (argc > 4) {
        gaze_pos_y = atof(argv[4]);
    }
    if (argc > 5) {
        gaze_pos_z = atof(argv[5]);
    }
    if (argc > 6) {
        gaze_dir_x = atof(argv[6]);
    }
    if (argc > 7) {
        gaze_dir_y = atof(argv[7]);
    }
    if (argc > 8) {
        gaze_dir_z = atof(argv[8]);
    }
    ...
    if (argc > 1 && (strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0)) {
        ...
        printf("\nDefault values:\n");
        printf("  input_file: longdress_vox10_1052.ply\n");
        printf("  output_file: theia_clean_output.ply\n");
        printf("  gaze_pos: (0.0, 1.5, -1.0)\n");
        printf("  gaze_dir: (0.0, 0.0, -1.0)\n");
        printf("\nExamples:\n");
        printf("  %s myfile.ply\n", argv[0]);
        printf("  %s myfile.ply output.ply\n", argv[0]);
        printf("  %s myfile.ply output.ply 0.0 1.5 -1.0 0.0 0.0 -1.0\n", argv[0]);
        return 0;
    }
    ...
    printf("Sample face point color: RGB=(%d,%d,%d)\n", points[456296].r, points[456296].g, points[456296].b);

// 1. Check Z-orientation of points
...
// 2. If most points are in negative Z half, invert them
if (count_neg_z > count_pos_z * 2) {
    printf(">>> Mirroring detected! Inverting Z coordinates...\n");
    for (int i = 0; i < numPoints; i++) {
        points[i].z = -points[i].z;
    }
} else if (count_pos_z > count_neg_z * 2) {
    printf(">>> Z coordinates are mostly positive (no mirroring).\n");
} else {
    printf(">>> Z coordinates are mixed (no clear mirroring).\n");
}
    // 2. Set Gaze from command-line arguments
    Gaze gaze;
    ...
    // 5. Fixed parameters for Test 3
    bool dynamic_skip = false;
    float rate_adapt = 1.0f;
    bool augmentation = false;

    printf("Calling get_fovea with rate_adapt=%.1f, augmentation=%d\n", rate_adapt, augmentation);
    ...
    printf("\nRESULTS:\n");
    printf("  Time: %llu ms\n", (end - start));
    printf("  Inner points: %lu\n", ret_points_inner.size());
    printf("  Outer points: %lu\n", ret_points_outer.size());

    // 6. Save file (without any additional filter)
    if (!ret_points_inner.empty()) {
        FILE* plyFile = fopen(output_filename, "w");
        if (plyFile) {
            // PLY header
            fprintf(plyFile, "ply\n");
            fprintf(plyFile, "format ascii 1.0\n");
            fprintf(plyFile, "comment Theia output (no post-filter)\n");
            fprintf(plyFile, "element vertex %lu\n", ret_points_inner.size());
            ...
            // Data (only clamp colors)
            for (const auto& p : ret_points_inner) {
                int r = p.r; if (r < 0) r = 0; if (r > 255) r = 255;
                int g = p.g; if (g < 0) g = 0; if (g > 255) g = 255;
                int b = p.b; if (b < 0) b = 0; if (b > 255) b = 255;
                fprintf(plyFile, "%.6f %.6f %.6f %d %d %d\n", p.x, p.y, p.z, r, g, b);
            }
            fclose(plyFile);
            printf("  Saved to '%s'\n", output_filename);
            
            // Display center coordinates of points (average)
            float avgX = 0, avgY = 0, avgZ = 0;
            for (const auto& p : ret_points_inner) {
                avgX += p.x; avgY += p.y; avgZ += p.z;
            }
            avgX /= ret_points_inner.size();
            avgY /= ret_points_inner.size();
            avgZ /= ret_points_inner.size();
            printf("  Average position: (%.3f, %.3f, %.3f)\n", avgX, avgY, avgZ);
        }
    }
